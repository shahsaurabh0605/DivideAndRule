	...
	for {
		...
		for sockThis, node_ := range serverThis.Socks {
			if node_.IsNew == true {
				...
				for keys, job_ := range serverThis.Jobs {
					if job_.IsScheduled == false {
						
						job_.IsScheduled = true
						job_.Sock = sockThis
						serverThis.Jobs[keys] = job_
						
						// fmt.Printf("After isScheduled check: %s \n", job_.Id)
						fmt.Printf("Job: %s picked up by node: %s \n", job_.Id, node_.Sock.Addr())

						// check if this job is picked by another socket
						// _, ok := serverThis.X[job_.Id]
						// if ok{						
						// 	break
						// }
						...
						if (err != nil && strings.Compare(jobRecieveResponse.IsOkay, "Okay") != 0) {
							fmt.Println(err)
						} else {
...
func onAcceptConnection(sock *gotalk.Sock) {
	...
	sock.CloseHandler = func(s *gotalk.Sock, _ int) {
		fmt.Println("close socket: %s", sock.Addr())
		//fmt.Printf("set: ", serverThis.X)
		serverThis.RWJobs.Lock()
		for job_id, job_ := range serverThis.Jobs {
			// fmt.Printf("Is Scheduled: ", job_.IsScheduled)
			if job_.IsScheduled == true && job_.Sock == s {
				job_.IsScheduled = false
				job_.Sock = nil
				// fmt.Printf("set: ", serverThis.X)
				// delete(serverThis.X, job_.Id)
